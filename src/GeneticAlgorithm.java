...
public class GeneticAlgorithm 
{
	...
	private PID_individual[] selection(PID_individual[] population)
	{
		...
		//mozna by to zapisac bez zbednych zmiennych, no ale niech bedzie czytelniej
	...
	private PID_individual[] crossover(PID_individual[] population)
	{
		...
		for(int i=0; i<population.length; i++)
		{
			...
			if(r.nextInt(2) == 0) // [first.p, second.i, second.d]
			{
				populationAfterCrossover[i] = new PID_individual(firstAdult.getP(), secondAdult.getI(), secondAdult.getD());
			}
			else // [second.p, first.i, first.d]
			{
				populationAfterCrossover[i] = new PID_individual(secondAdult.getP(), firstAdult.getI(), firstAdult.getD());
			}
		}
	...
	private PID_individual[] mutation(PID_individual[] population)
	{
		...
		for(int i=0; i<population.length; i++)
		{
			if(r.nextInt(101) <= this.mutation_probability)
			{
				...
				int multiplier = StartPopulationGenerator.power(10, r.nextInt(3));
